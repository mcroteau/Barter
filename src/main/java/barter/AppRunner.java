...
public class AppRunner {
	...
	private boolean createMocks(){
		...
		for(String name: prospectNames){
			Status status = statuses.get(barter.common.Barter.getNumber(statuses.size() - 1));
			Prospect prospect = new Prospect();
			prospect.setName(name);
			prospect.setEmail(barter.common.Barter.getString(13));
			prospect.setPhone(Integer.toString(barter.common.Barter.getNumber(10)));
		...
		for(Prospect prospect: prospects){
			int index = barter.common.Barter.getNumber(activityNames.length - 1);
		...
		for(int z = 0; z < 25; z++){
			Status startingStatus = statuses.get(barter.common.Barter.getNumber(statuses.size() - 1));

			Prospect prospect = prospects.get(barter.common.Barter.getNumber(prospectNames.length - 1));
			Effort effort = new Effort();
			effort.setProspectId(prospect.getId());
			effort.setStartingStatusId(startingStatus.getId());
			effort.setStartDate(barter.common.Barter.getYesterday(barter.common.Barter.getNumber(31)));
			Effort savedEffort = effortRepo.save(effort);

			for(int k = 0; k < barter.common.Barter.getNumber(13); k++){
				int index = barter.common.Barter.getNumber(activityNames.length - 1);
				Activity activity = activities.get(index);
				ProspectActivity prospectActivity = new ProspectActivity();
				...
				prospectActivity.setActivityId(activity.getId());
				prospectActivity.setProspectId(prospect.getId());
				prospectActivity.setCompleteDate(barter.common.Barter.getDate());
				if(k % 2 == 0) {
			...
			savedEffort.setEndDate(barter.common.Barter.getDate());
			effortRepo.update(savedEffort);

			log.info("Efforts : " + effortRepo.getCount());
			log.info("Customer Activities : " + effortRepo.getActivityCount());
		}
